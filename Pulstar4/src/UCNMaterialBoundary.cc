...
const G4Step& aStep)
{
  ...
  G4double momperp = mom.dot(theGlobalNormal);
  G4double mompar = sqrt(mom.dot(mom) - momperp*momperp);

  G4ThreeVector momperp_vec = momperp*theGlobalNormal;
  G4ThreeVector mompar_vec = mom - momperp_vec;

  G4double velnorm = vel * tfakt;
  G4double eperp = momperp*momperp/2./neutron_mass_c2/neV; 
  G4double epar = mompar*mompar/2./neutron_mass_c2/neV; 
  ...
  //  if(fabs(fermipot_diff) < 1.0e-3){
    ...
    static long count(0);
    G4bool PotComp(false);
    PotComp = comparepot(eperp, fermipot_diff);  //1: eperp > fermipot_diff; 0: eperp < fermipot_diff
    
    if (PotComp == 0){             // reflect from surface
    ...
    /////// above critical velocity
      ...
      if (just_reflected == 0){ // transmit material, first check whether there will be another reflection
	...
	G4double refl2 = reflectivity(0, fermipot_diff,eperp);
	...
	{   
	  ...
	  G4double eperpnew = transmit(fermipot_diff, eperp);
	  
	  G4double enew = epar + eperpnew;
	  G4double pperpnew = sqrt(2.0*neutron_mass_c2*eperpnew*neV);

	  G4double pperpnew_sign(0.0);

	  if(momperp){
	    pperpnew_sign = momperp/fabs(momperp);
	    pperpnew *= pperpnew_sign;
	  }
	  else{
	    std::cerr << "momperp  is zero!!!" << std::endl;
	    exit(1);
	  }
	    
	  //	  theGlobalNormal = -theGlobalNormal;

	  G4ThreeVector pperpnew_vec = pperpnew*theGlobalNormal;
	  G4ThreeVector momnew_vec = pperpnew_vec + mompar_vec;

	  //	  G4double m = -sqrt(momperp*momperp - neutron_mass_c2*2.*fermipot_diff*neV);
	  
	  // --- momentum direction in new media
	  //	  G4ThreeVector ref = mom - (momperp-m)*theGlobalNormal;
	  //	  G4ThreeVector ref = mom - (pperpnew-m)*theGlobalNormal;
	  //	  aParticleChange.ProposeMomentumDirection(ref.unit());


	  aParticleChange.ProposeMomentumDirection(momnew_vec.unit());
...
G4double UCNMaterialBoundary::reflectivity(int, G4double fpot, G4double eperp){
  
  G4double r = (sqrt(eperp) - sqrt(eperp - fpot)) /
  (sqrt(eperp) + sqrt(eperp - fpot));
...
G4ThreeVector UCNMaterialBoundary::cos_diff(G4ThreeVector localnormal)
{
  ...
  //rotateUz -> Rotates axes so that the Z axis is along localnormal
  momentum.rotateUz(localnormal);
  if(momentum*localnormal<0) {
    momentum*=-1;
    G4cout << "!" << G4endl;}
  return momentum;
