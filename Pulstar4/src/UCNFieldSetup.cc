...
//
//
// $Id: UCNFieldSetup.cc,v 1.2 2005/02/08 10:57:28 kuzniak Exp $
// GEANT4 tag $Name:  $
//
//  
...
{
  //  fEMfield = new UCNUniformGravField(G4ThreeVector(0.0, 0.0, -4.8979e-9*6.673e-2*volt/cm)); // + = backward, - = forward
  fEMfield = new UCNUniformGravField(G4ThreeVector(0.0, 4.8979e-9*6.673e-2*volt/cm, 0.0)); // + = down
  //  fEMfield = new UCNUniformGravField(G4ThreeVector(4.8979e-9*6.673e-2*volt/cm, 0.0, 0.0)); // + = left
  //  fEMfield = new UCNUniformGravField(G4ThreeVector(0.0, 0.0, 0.0));
 ...
 fMinStep     = 0.00000010*mm ; // minimal step of 10 microns
 ...
 G4double minEps(1.0e-6*mm);
 G4double maxEps(1.5e-6*mm);
 fFieldManager->SetMinimumEpsilonStep( minEps );
 fFieldManager->SetMaximumEpsilonStep( maxEps );
 fFieldManager->SetDeltaOneStep( 0.5e-6 * mm ); 
 UpdateField();
...
{    
  ...
  //timedep->SetFieldEqPointer((UCNUniformGravField*)fEMfield);
